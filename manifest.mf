...
public class Calculadora extends javax.swing.JFrame {
    ...
    private String ultimaOperacion = ""; // Variable para almacenar la última operación
    ...
    private void initComponents() {
        ...
        btn_ultimaoperacion = new javax.swing.JButton();
        ...
        btn_ultimaoperacion.setFont(new java.awt.Font("Montserrat Alternates Medium", 0, 21)); // NOI18N
        btn_ultimaoperacion.setForeground(new java.awt.Color(55, 62, 71));
        btn_ultimaoperacion.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1.png"))); // NOI18N
        btn_ultimaoperacion.setText("L");
        btn_ultimaoperacion.setFocusPainted(false);
        btn_ultimaoperacion.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        btn_ultimaoperacion.setPreferredSize(new java.awt.Dimension(50, 50));
        btn_ultimaoperacion.setPressedIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1.png"))); // NOI18N
        btn_ultimaoperacion.setRolloverIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1_pressed.png"))); // NOI18N
        btn_ultimaoperacion.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_ultimaoperacionActionPerformed(evt);
            }
        });
        jPanel2.add(btn_ultimaoperacion, new org.netbeans.lib.awtextra.AbsoluteConstraints(180, 20, -1, -1));
        ...
        btn_historial.setFont(new java.awt.Font("Montserrat Alternates Medium", 0, 21)); // NOI18N
        btn_historial.setForeground(new java.awt.Color(55, 62, 71));
        btn_historial.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1.png"))); // NOI18N
        ...
        btn_historial.setPressedIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1.png"))); // NOI18N
        btn_historial.setRolloverIcon(new javax.swing.ImageIcon(getClass().getResource("/images/btn1_pressed.png"))); // NOI18N
        btn_historial.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btn_historialActionPerformed(evt);
            }
        });
        jPanel2.add(btn_historial, new org.netbeans.lib.awtextra.AbsoluteConstraints(180, 340, -1, -1));
    ...
    private void btn_ultimaoperacionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ultimaoperacionActionPerformed
        // Verificar si hay operaciones en el historial
        if (!historialOperaciones.isEmpty()) {
            // Eliminar la última operación del historial
            historialOperaciones.remove(historialOperaciones.size() - 1);
            JOptionPane.showMessageDialog(this, "Última operación eliminada del historial.", "Historial", JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "No hay operaciones en el historial.", "Historial", JOptionPane.WARNING_MESSAGE);
        }

    }//GEN-LAST:event_btn_ultimaoperacionActionPerformed
    ...
    private void btn_igualActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_igualActionPerformed
        try {
            ...
            if (!operacion.isEmpty()) {
                // Si hay una operación nueva, calcularla
                Expression expression = new ExpressionBuilder(operacion).build();
                resultado = expression.evaluate();
                ultimaOperacion = operacion; // Guardar la última operación
            } else {
                // Si no hay operación nueva, repetir la última operación con el resultado anterior
                if (!ultimaOperacion.isEmpty() && !txtResultado.getText().isEmpty()) {
                    String resultadoAnterior = txtResultado.getText();
                    // Extraer el operador y el segundo operando de la última operación
                    String operador = "";
                    String operando = "";
                    for (int i = 0; i < ultimaOperacion.length(); i++) {
                        char c = ultimaOperacion.charAt(i);
                        if (c == '+' || c == '-' || c == '*' || c == '/') {
                            operador = String.valueOf(c);
                            operando = ultimaOperacion.substring(i + 1);
                            break;
                        }
                    }
                    if (!operador.isEmpty() && !operando.isEmpty()) {
                        // Construir la nueva operación: resultado anterior + operador + operando
                        operacion = resultadoAnterior + operador + operando;
                        Expression expression = new ExpressionBuilder(operacion).build();
                        resultado = expression.evaluate();
                    } else {
                        throw new Exception("No se puede repetir la operación.");
                    }
                } else {
                    throw new Exception("No hay operación para repetir.");
                }
            }

            // Formatear el resultado a solo 2 decimales
            DecimalFormat formato = new DecimalFormat("#.##");
            String resultadoFormateado = formato.format(resultado);

            // Mostrar el resultado
            txtResultado.setText(String.valueOf(resultadoFormateado));

            // Guardar la operación en el historial
            historialOperaciones.add(operacion + " = " + resultadoFormateado);

            // Limpiar la operación actual para permitir repeticiones
            txtOperacion.setText("");
        } catch (Exception e) {
            txtResultado.setText("Syntaxis Error");
        }                                 
    ...
    private void btn_oscuroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_oscuroActionPerformed
        if (!modoOscuro) {
            ...
            cambiarColorBtn1(btn_ultimaoperacion);
            cambiarColorBtn1(btn_historial);
    ...
    private void btn_historialActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_historialActionPerformed
        ...
        for (String operacion : historialOperaciones) {
            historial.append(operacion).append("\n");
        }
        JOptionPane.showMessageDialog(this, historial.toString(), "Historial de Operaciones", JOptionPane.INFORMATION_MESSAGE);
    ...
    private javax.swing.JButton btn_ultimaoperacion;
